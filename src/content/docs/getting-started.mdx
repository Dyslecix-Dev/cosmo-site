...
<Callout type="note" title="Why no bun variant?">
  Astro runs on bun, but it's not a fully supported runtime. Some Vite/Astro internals still depend on Node APIs, and bun's npm-compatible install behavior occasionally drifts from npm/pnpm. See Astro's [bun recipe](https://docs.astro.build/en/recipes/bun/) if you want to try it.
</Callout>
